...
namespace Mirle.SMTCV.Conveyor.V2BYMA30.SMT.Signal
{
    public class SignalMapper
    {
        ...
        {
            new BlockInfo(new DDeviceRange("D1001", "D5030"), "ReadWrite", 0),
        };

        public readonly static int BufferCount = 50;
        ...
        private void MappingBuffer()
        {
            ...
            for (int i = 1; i <= BufferCount; i++)
            {
                ...
                {
                    bufferSignal.BCRResultSignal.TrayID = new BCRResult();
                    bufferSignal.BCRResultSignal.TrayID.ID = new WordBlock(_mplc, $"D{bcrStartAddress + 6}", 5);
                }
                if (i == 13)
                {
                    bufferSignal.BCRResultSignal.TrayID = new BCRResult();
                    bufferSignal.BCRResultSignal.TrayID.ID = new WordBlock(_mplc, $"D{bcrStartAddress + 11}", 5);
                }
                if (i == 19)
                {
                    bufferSignal.BCRResultSignal.TrayID = new BCRResult();
                    bufferSignal.BCRResultSignal.TrayID.ID = new WordBlock(_mplc, $"D{bcrStartAddress + 16}", 5);
                }
                if (i == 25)
                {
                    bufferSignal.BCRResultSignal.TrayID = new BCRResult();
                    bufferSignal.BCRResultSignal.TrayID.ID = new WordBlock(_mplc, $"D{bcrStartAddress + 21}", 5);
                }
                if (i == 31)
                {
                    bufferSignal.BCRResultSignal.TrayID = new BCRResult();
                    bufferSignal.BCRResultSignal.TrayID.ID = new WordBlock(_mplc, $"D{bcrStartAddress + 26}", 5);
                }
