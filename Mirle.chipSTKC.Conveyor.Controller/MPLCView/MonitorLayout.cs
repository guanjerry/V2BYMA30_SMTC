...
namespace Mirle.SMTCV.Conveyor.Controller.MPLCView
{
    public partial class MonitorLayout : Form
    {
        ...
        public MonitorLayout(CVCManager_8F _CVCManager, CVCManager_S800 _CVCManager_S800,  int curC)
        {
            InitializeComponent();
        ...
        private void refresherTimer_Tick(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    for (int i = 1; i <= _cvcManager_S800.GetSignalMapper().GetBufferCount(); i++)
                    {
                ...
                else
                {
                    for (int i = 1; i <= _cvcManager_S800.GetSignalMapper().GetBufferCount(); i++)
                    {
                        var buffer = _cvcManager_S800.GetBuffer(i);
                        uclBuffer BufferControl = Controls.Find("ucl" + showBuff[i - 1], true).FirstOrDefault() as uclBuffer;
        ...
        {
            V2BYMA30.SMT.Buffer buffer = _cvcManager.GetBuffer(index);
