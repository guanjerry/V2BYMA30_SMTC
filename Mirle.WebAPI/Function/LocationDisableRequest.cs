...
{
    public class LocationDisableRequest
    {
        private WebApiConfig _config = new WebApiConfig();

        public LocationDisableRequest(WebApiConfig Config)
        {
            _config = Config;
        }

        public bool FunReport(LocationDisableInfo info)
        {
            try
            {
                string strJson = JsonConvert.SerializeObject(info);
                clsWriLog.Log.FunWriTraceLog_CV(strJson);
                string sLink = $"http://{_config.IP}/LOCATION_DISABLE_REQUEST";
