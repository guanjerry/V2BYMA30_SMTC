...
{
    public class BlockStsChangeInform
    {
        private WebApiConfig _config = new WebApiConfig();

        public BlockStsChangeInform(WebApiConfig Config)
        {
            _config = Config;
        }

        public bool FunReport(BlockStsChangeReport info)
        {
            try
            {
                string strJson = JsonConvert.SerializeObject(info);
                clsWriLog.Log.FunWriTraceLog_CV(strJson);
                string sLink = $"http://{_config.IP}/BLOCK_STS_CHANGE_INFORM";
