...
{
    public class ControlChangeInform
    {
        private WebApiConfig _config = new WebApiConfig();

        public ControlChangeInform(WebApiConfig Config)
        {
            _config = Config;
        }

        public bool FunReport(ControlChangeReport info)
        {
            try
            {
                string strJson = JsonConvert.SerializeObject(info);
                clsWriLog.Log.FunWriTraceLog_CV(strJson);
                string sLink = $"http://{_config.IP}/CONTROL_CHANGE_INFORM";
