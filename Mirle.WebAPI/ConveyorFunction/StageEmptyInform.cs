...
{
    public class StageEmptyInform
    {
        private WebApiConfig _config = new WebApiConfig();

        public StageEmptyInform(WebApiConfig Config)
        {
            _config = Config;
        }

        public bool FunReport(StageEmptyReport info)
        {
            try
            {
                string strJson = JsonConvert.SerializeObject(info);
                clsWriLog.Log.FunWriTraceLog_CV(strJson);
                string sLink = $"http://{_config.IP}/STAGE_EMPTY_INFORM";
