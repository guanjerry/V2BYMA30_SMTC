...
{
    public class RackCommandDoneInform
    {
        private WebApiConfig _config = new WebApiConfig();

        public RackCommandDoneInform(WebApiConfig Config)
        {
            _config = Config;
        }

        public bool FunReport(RackCommandDoneInfo info)
        {
            try
            {
                string strJson = JsonConvert.SerializeObject(info);
                clsWriLog.Log.FunWriTraceLog_CV(strJson);
                string sLink = $"http://{_config.IP}/RACK_COMMAND_DONE";
