...
{
    public class SmtEmptyMagLoadRequest
    {
        private WebApiConfig _config = new WebApiConfig();

        public SmtEmptyMagLoadRequest(WebApiConfig Config)
        {
            _config = Config;
        }

        public bool FunReport(SmtEmptyMagLoadRequestInfo info)
        {
            try
            {
                string strJson = JsonConvert.SerializeObject(info);
                clsWriLog.Log.FunWriTraceLog_CV(strJson);
                string sLink = $"http://{_config.IP}/WCS/SMTC_EMPTY_MAGAZINE_LOAD_REQUEST";
