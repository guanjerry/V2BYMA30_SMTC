...
{
    public class SmtMagLoadRequest
    {
        private WebApiConfig _config = new WebApiConfig();

        public SmtMagLoadRequest(WebApiConfig Config)
        {
            _config = Config;
        }

        public bool FunReport(SmtMagLoadRequestInfo info)
        {
            try
            {
                string strJson = JsonConvert.SerializeObject(info);
                clsWriLog.Log.FunWriTraceLog_CV(strJson);
                string sLink = $"http://{_config.IP}/WCS/SMTC_MAGAZINE_LOAD_REQUEST";
